...
var RenameOnYAMLSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Rename On YAML Settings" });
  ...
  createFolderSetting(containerEl, setting, index) {
    new import_obsidian.Setting(containerEl).setName(`Folder Setting ${index + 1}`).setDesc("Set the folder path and rename pattern for files in this folder.").addText((text) => text.setPlaceholder("Enter folder path").setValue(setting.folderPath).onChange(async (value) => {
      setting.folderPath = value;
      await this.plugin.saveSettings();
    })).addText((text) => text.setPlaceholder("Enter file name template").setValue(setting.fileNameTemplate).onChange(async (value) => {
      if (ILLEGAL_CHARACTERS.test(value)) {
        new import_obsidian.Notice("The file name template contains illegal characters.");
      } else {
...
// src/main.ts
var ILLEGAL_CHARACTERS2 = /[\\/:*?"<>|]/;
var DEFAULT_SETTINGS = {
  folderSettings: []
};
var RenameOnYAMLPlugin = class extends import_obsidian2.Plugin {
  async onload() {
      ...
      for (const folderSetting of this.settings.folderSettings) {
        if (file.path.startsWith(folderSetting.folderPath)) {
          ...
          if (file.name !== newFileName) {
            ...
            await this.app.vault.rename(file, `${filePath}/${newFileName}`);
          }
          break;
        }
      }
